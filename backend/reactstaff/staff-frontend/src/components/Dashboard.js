    ...
    <div style={{display: 'flex', justifyItems:'center', flexDirection: 'column'}}>
      ...
      {(!isFree && !sentOrder) && (
        <div align="center" id="wait-container">
          <h1 align="center" sx={{mt: 2}}> You are making an Order </h1>
          <div style={{display: 'flex', flexDirection: 'column', alignItems: 'start', margin: '0 30vw 24px'}}>
            <p><b>Client: </b> {order.name}</p>
            <p><b>Location Tag: </b> {order.locationTag}</p>
            <p><b>Order: </b></p>
          </div>
            <DataGrid
            ...
            />
            <Button sx={{my: '16px'}} variant="contained" color="success" onClick={sendRobot}>Complete Order</Button>
        </div>
      )}
      
      {sentOrder && (
        <div style={{marginTop: '300px'}} align="center" id="robot-container">
          {robotTime ? (<h1 align="center" sx={{mt: 2}}> Robot dispatched for Location Tag #343</h1>) 
          :
          (          
            <div>
              <h1 align="center" sx={{mt: 2}}> Robot arrived </h1>
              <Button sx={{my: '16px'}} variant="contained" color="success" onClick={() => {
                setSentOrder(false)
                setIsFree(true)
                setRobotTime(true)
